...
def load_artifacts():
    ...
    # Load Target Scaler (used to un-scale CNN-BiLSTM output)
    try:
        scaler_y = joblib.load('scaler_y.pkl')
    except Exception:
        pass  # Optional target scaler

    # Lazy-load TensorFlow/Keras so a missing/broken TF install doesn't crash the whole app
    try:
        import tensorflow as tf
        if os.path.exists('cnn_bilstm_model.keras'):
            dl_model = tf.keras.models.load_model('cnn_bilstm_model.keras')
        elif os.path.exists('cnn_bilstm_model.h5'):
            dl_model = tf.keras.models.load_model('cnn_bilstm_model.h5')
    except Exception:
        st.info("ℹ️ TensorFlow/DL model not available — the sequence-forecast tab will fall back to the XGBoost model.")
                ...
                if st.button("Run Sequence Forecast"):
                    ...
                    if scaler_X is not None:
                        ...
                        if dl_model is not None:
                            ...
                            st.info("ℹ️ DL Model file (`cnn_bilstm_model.keras`) not found. Running baseline inference on the most recent reading instead:")
                            if xgb_model is not None:
